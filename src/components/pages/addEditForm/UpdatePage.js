...
const UpdatePage = memo(() => {
  ...
  const fetchSpeciesData = async () => {
    try {
      const response = await axios.get(`${Api.apiLoai}/${id}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(data);
      setTen(data.ten);
      setTenkhoahoc(data.ten_khoa_hoc);
      setTentacgia(data.ten_tac_gia);
      setTendiaphuong(data.ten_dia_phuong);
      setNguondulieu(data.nguon_du_lieu);
      setKingid(data.kingdom_id);
      setPhylumid(data.phylum_id);
      setClassid(data.class_id);
      setOderid(data.order_id);
      setFamilyid(data.family_id);
      setGenusid(data.genus_id);
      setNamreddbook(
        data.sach_dos[0] && data.sach_dos[0].pivot
          ? data.sach_dos[0].pivot.nam
          : ""
      );
      setSinhcanhid(
        data.sach_dos[0] && data.sach_dos[0].id ? data.sach_dos[0].id : ""
      );
      setNamiucn(
        data.iucns[0] && data.iucns[0].pivot ? data.iucns[0].pivot.nam : ""
      );
      setSuygiamid(data.iucns[0] && data.iucns[0].id ? data.iucns[0].id : "");
    };
  ...
  const handleUpdateSpecies = async (e) => {
    ...
    try {
      const updatedData = {
        ...
        iucns: namIucn && suyGiamid ? [{ nam: namIucn, id: suyGiamid }] : [],
        kingdom_id: kingId,
        loai_noi_bat: false,
      ...
      await axios.put(`${Api.apiLoai}/${id}`, updatedData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
